...
- v1.4.3 20260413 The Windows Media Player "jumping playhead" has been fixed. The script now inserts silent audio during the countdown-sequence so video-stream and audio-stream always begin at the same time whether countdown-intro is used or not.
- v1.4.2 20260413 Removed the "video only" function as it wasn't implemented correctly and didn't work. I also changed the image-generation sequence so that all generated images are now, as they are supposed to be, double-height which is necessary for smooth-looking scroll (It simulates sub-pixel smoothness by scaling the image down before generating video). This modification does not change how you should use the image-sequence in a video-editor, where you should still use "stretch" to make sure the image fits your desired aspect-ratio (The width is normal, but the height is double, so "stretch" will compress the images, similar to if they had been filmed with an anamorph filter). There is currently a known issue where the video sometimes jumps the playhead past the countdown-sequence. This will (Hopefully) be fixed later, as it is not a major issue and won't cause any trouble when the video is loaded into a video-editor (It's just an issue with Windows Media Player not handling the video as one single coherent stream. The cause is how audio is output by the script, which doesn't match video-length due to the countdown and end-padding that is being applied)
