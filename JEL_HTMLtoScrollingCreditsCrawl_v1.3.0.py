...
# CONFIG
# -----------------------------
APP_TITLE = "JELSTUDIO HTMLtoScrollingCreditsCrawl v1.3.0"
...
def create_scroll_frames_parallel(image_path, out_dir, video_size, fps, duration, use_ease, bg_color, start_index, progress_cb=None):
    ...
    # Robust resume: detect existing frames
    existing_frames = set()
    ...
    with ThreadPoolExecutor(max_workers = min(MaxCPUcoresToUseToPreventDiskThrashing, os.cpu_count())) as executor:
        futures = []
        for i in range(total_frames):
            global_index = start_index + i
            
            if global_index in existing_frames:
                continue
...
def create_countdown(out_dir, video_size, fps, seconds, bg_color, start_index):
    ...
    try:
        ...
        print(f"Successfully loaded font: {FONT_FILE_NAME}")
    except IOError:
        print(f"ERROR: Font file '{FONT_FILE_NAME}' not found! Falling back to tiny default font.")
...
class App:
    def __init__(self, root):
        ...
        self.use_countdown = BooleanVar(value=True)  # New option
        self.keep_images = BooleanVar(value=True)
        self.images_only = BooleanVar(value=False)
        self.video_only = BooleanVar(value=False)
        ...
        Checkbutton(root, text="Use Countdown", variable=self.use_countdown).pack()  # New GUI element
        ...
        Checkbutton(root, text="Video Only", variable=self.video_only).pack()
    ...
    def run(self):
        if not self.html_path and not self.video_only.get():
            self.status.config(text="Select HTML file or check 'Video Only'")
            return

        if not self.image_root:
            self.status.config(text="Select image folder")
            return
        ...
        try:
            ...
            self.status.config(text="ERROR: Invalid duration value.")
            return
        
        if self.audio_path and float(self.duration.get()) <= 0:
             # This should ideally not happen if load_audio works, but good defensive check.
             self.status.config(text="ERROR: Invalid duration calculated.")
             return
           
        if self.audio_path:
            self.status.config(text=f"Audio length detected: {scroll_duration:.2f}s. Scroll time set to match music.")
        else:
            self.status.config(text=f"Using manual scroll duration: {scroll_duration:.2f} seconds.")
        ...
        if not self.video_only.get():
            ...
            # Ensure we have the HTML path before trying to render the background
            if not self.html_path:
                self.status.config(text="ERROR: Video generation requires an HTML file.")
                return
                
            # bg_color is guaranteed to be defined if we enter this block
            self.status.config(text="Rendering HTML...")
            img_path = os.path.join(frames_dir, "full.png")
            bg_color = render_html_fullpage(self.html_path, img_path, size[0]) # DEFINED HERE

            # 1. Countdown Phase Seconds
            if self.use_countdown.get():
                self.status.config(text=f"Countdown ({COUNTDOWN_TIME}s)...")
                frame_index = create_countdown(frames_dir, size, fps, COUNTDOWN_TIME, bg_color, frame_index)
            else:
                # Skip countdown and intro padding entirely
                self.status.config(text="Skipping countdown and intro padding...")
                # Create a single blank frame at the correct resolution to maintain aspect ratio
                frame = Image.new("RGB", (size[0], size[1]), bg_color)
                frame.save(os.path.join(frames_dir, f"frame_{frame_index:06d}.png"))
                frame_index += 1

            # 2. Scrolling Content Phase (DYNAMIC)
            self.status.config(text="Rendering scrolling text frames...")
            frame_index = create_scroll_frames_parallel(
                img_path, frames_dir, size, fps, scroll_duration, # Uses dynamically set duration
                self.use_ease.get(), bg_color,
                frame_index,
                progress_cb=self.update_progress
            )

            # 3. End Padding Phase Seconds
            self.status.config(text=f"End padding ({PADDING_TIME}s)...")
            frame_index = create_end_padding(frames_dir, size, fps, PADDING_TIME, bg_color, frame_index)

        if not self.images_only.get():
            self.status.config(text="Encoding video...")
        ...
        if not self.keep_images.get():
            self.status.config(text="Cleaning up...")
